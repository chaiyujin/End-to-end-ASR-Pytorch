...
class TimitDataset(Dataset):
    def __init__(self, file_path, sets, bucket_size, max_timestep=0, max_label_len=0):
        ...
        indices = list(reversed(np.argsort(sortd_len)))
        sorted_x = [x[idx] for idx in indices]
        sorted_y = [y[idx] for idx in indices]
        ...
        for b in range(int(np.ceil(len(sorted_x)/bucket_size))):
            ...
            self.X_len.append(np.asarray([
                len(t) for t in sorted_x[offset:bound]
            ], dtype=np.int64))
